...
1. *`Аутентификация пользователя. Пользователь подтверждает личность в сервисе оператора/платёжном интерфейсе.`*

2. *`BEGIN TRANSACTION Открывается транзакция в БД оператора связи.`*

3. *`Валидация данных. Проверка номера телефона, суммы, статуса абонента/договора.`*

4. *`Операция пополнения. UPDATE/увеличение баланса лицевого счёта абонента на указанную сумму.`*

5. *`Регистрация платежа INSERT записи о пополнении в историю/реестр платежей со статусом.`*

6. *`COMMIT Фиксация транзакции.`*

*`При сбое на шагах 3–5 выполняется ROLLBACK.`*
